<?php 
	...
	//se da que está definida cuando el usuario inicia sesion, ya que la envía en la petición
	if (isset($_POST['registrarse'])) {
		...
		if ($crud->buscarUsuario($_POST['usuario'])) {
			...
			header('Location: ../Templates/Error.php?mensaje=Nombre de usuario en existencia');
		}		
		...
		if ($usuario->getId()!=NULL AND $usuario->getTipo()!="operador") {
			...
			header('Location: ../Templates/Error.php?mensaje=Operacion fallida: usuario o contraseña incorrectos'); // cuando los datos son incorrectos envia a la página de error
		}
